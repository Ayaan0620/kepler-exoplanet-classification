...
# --- our logistic regression (from scratch, no sklearn) ---
...
# --- load data ---
# NaN values kept intact, imputation happens inside CV folds
print('loading data...')
df = pd.read_csv('../data/kepler_clean_v2.csv')
y = df['target'].values
X_raw = df.drop('target', axis=1)

print(f'dataset: {X_raw.shape[0]} rows x {X_raw.shape[1]} features')
print(f'classes: {(y==1).sum()} confirmed, {(y==0).sum()} false positives')
# print(X_raw.head())  # debugging
print(f'NaN cells: {X_raw.isna().sum().sum()}')
...
# all preprocessing inside the loop to prevent leakage
...
for fs_name, fs_cols in feature_sets.items():
    ...
    for train_idx, test_idx in skf.split(X_raw, y):
        ...
        # feature engineering on each fold separately
        ...
        # pick columns for this feature set
        X_train_fs = X_train_df[fs_cols].copy()
        X_test_fs  = X_test_df[fs_cols].copy()

        # impute NaN using median (fit on train only!)
...
# --- print results ---
print('\n--- Results (leakage-free, 10-fold CV) ---')
...
print('\nsaved experiment_results_v3.csv')
...
print(f'  uncertainty only: {xgb_unc:.4f}')
print(f'  raw + unc:        {xgb_both:.4f}')
gap = xgb_unc - xgb_raw
print(f'  gap (unc - raw):  {gap:+.4f}')
# TODO: maybe add a statistical test here to check significance

within = abs(gap) <= 0.01
print(f'\nuncertainty-only within 0.01 AUC of raw-only? {"yes" if within else "no"} (gap = {gap:+.4f})')
...
plt.tight_layout()
plt.savefig('../results/figures/uncertainty_investigation.png', dpi=150)
plt.close()
